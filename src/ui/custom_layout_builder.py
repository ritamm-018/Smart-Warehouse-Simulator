...
def custom_layout_builder():
    ...
    st.markdown('<div class="section-title">🏗️ Custom Layout Builder</div>', unsafe_allow_html=True)
    st.markdown(f"<span style='color:#64748b;font-size:1.1rem;'>Current Grid Size: <b>{grid_width} x {grid_height}</b></span>", unsafe_allow_html=True)
    # Phase indicator
    phase = st.session_state.custom_layout_state['phase']
    if phase == 'shelves':
        st.markdown("""
        <div style='background:#e0e7ef;border-radius:8px;padding:0.8rem 1.2rem;margin-bottom:0.7rem;display:flex;align-items:center;'>
            <span style='font-size:1.5rem;margin-right:0.7rem;'>📦</span>
            <span><b>Phase 1: Place Shelves</b><br><span style='font-size:0.98rem;'>Click on grid cells to place/remove shelves</span></span>
        </div>
        ...
        <div style='background:#e0e7ef;border-radius:8px;padding:0.8rem 1.2rem;margin-bottom:0.7rem;display:flex;align-items:center;'>
            <span style='font-size:1.5rem;margin-right:0.7rem;'>📋</span>
            <span><b>Phase 2: Place Packing Stations</b><br><span style='font-size:0.98rem;'>Click on empty cells to place/remove packing stations</span></span>
        </div>
        """, unsafe_allow_html=True)
        ...
        <div style='background:#e0e7ef;border-radius:8px;padding:0.8rem 1.2rem;margin-bottom:0.7rem;display:flex;align-items:center;'>
            <span style='font-size:1.5rem;margin-right:0.7rem;'>🚪</span>
            <span><b>Phase 3: Place Entry/Exit Point</b><br><span style='font-size:0.98rem;'>Click on an empty cell to set entry/exit point</span></span>
        </div>
        """, unsafe_allow_html=True)
    ...
    if phase == 'shelves':
        ...
        with col1:
            if 'default_shelf_type' not in st.session_state:
                st.session_state['default_shelf_type'] = 'A'
            st.session_state['default_shelf_type'] = st.selectbox(
                "Select Shelf Type (applies to next shelf you place)",
                options=list(shelf_categories.keys()),
                format_func=lambda x: f"{x} - {shelf_categories[x]}",
                index=list(shelf_categories.keys()).index(st.session_state['default_shelf_type'])
            )
        with col2:
        ...
        with col3:
            if st.button("🔄 Reset Layout", 
                ...
                st.rerun()
        # Show legend below controls
        st.markdown("**Shelf Type Legend:** " + ", ".join([f"{k}: {v}" for k, v in shelf_categories.items()]))
    ...
    # Add buttons for different phases
    if phase == 'stations':
        ...
        with col2:
            if st.button("Done with Entry/Exit", 
                ...
                if st.session_state.custom_layout_state['entry_exit'] is not None:
                    ...
                    save_custom_layout()
                    # Store in warehouse_state
                    custom_state = st.session_state.custom_layout_state
                    warehouse_state['layout'] = {
                        'width': grid_width,
                        'height': grid_height,
                        'shelves': custom_state['shelves']  # Include shelf types
                    }
                    # Collect simulation parameters
                    st.markdown('---')
                    st.markdown('### Simulation Parameters')
                    num_pickers = st.number_input('Number of Pickers', min_value=1, max_value=10, value=3, step=1, key='sim_num_pickers')
                    picker_speed = st.selectbox('Picker Speed', ['Slow', 'Medium', 'Fast'], index=1, key='sim_picker_speed')
                    num_orders = st.number_input('Number of Orders', min_value=1, max_value=500, value=50, step=1, key='sim_num_orders')
                    items_per_order = st.number_input('Items per Order', min_value=1, max_value=20, value=5, step=1, key='sim_items_per_order')
                    simulation_speed = st.selectbox('Simulation Speed', ['1x', '2x', '5x', '10x'], index=2, key='sim_speed')
                    config = {
                    ...
                    warehouse_state['config'] = config
                    # Build config for run_simulation
                    sim_config = {
                        'grid_width': grid_width,
                        'grid_height': grid_height,
                        'shelf_positions': [(s['x'], s['y']) for s in custom_state['shelves']],
                        'packing_stations': [(s['x'], s['y']) for s in custom_state['stations']],
                        'num_workers': int(num_pickers),
                        'orders': [[(s['x'], s['y']) for s in custom_state['shelves'][:items_per_order]] for _ in range(num_orders)]
                    }
                    warehouse_state['simulation_results'] = run_simulation(sim_config)
                    st.success("✅ Custom layout, simulation config, and simulation results saved successfully!")
    ...
    for y in range(grid_height):
        ...
        for x in range(grid_width):
            with cols[x + 1]:
                ...
                # Create a container for the cell with text inside clickable buttons
                with st.container():
                    # Create clickable button with shelf type code or emoji
                    if cell_type == 'Shelf':
                        # Show the shelf type code (A, B, C, etc.)
                        button_text = cell_data.get('shelf_type', '')
                    elif cell_type == 'Packing Station':
                        button_text = "📋"
                    elif cell_type == 'Entry/Exit':
                        button_text = "🚪"
                    ...
                    # Clickable button with text inside
...
def create_interactive_grid():
    ...
    for element_type in df['type'].unique():
        ...
        if len(element_data) > 0:
            color_val = list(element_data['color'])[0]
            symbol_val = list(element_data['symbol'])[0]
...
def handle_grid_click(x, display_y):
    ...
    if phase == 'shelves':
        ...
        if grid_data[cell_idx]['type'] == 'Empty':
            # Add shelf with selected type
            selected_type = st.session_state.get('default_shelf_type', 'A')
            grid_data[cell_idx].update({'type': 'Shelf', 'color': 'brown', 'symbol': 'square', 'shelf_type': selected_type})
            st.session_state.custom_layout_state['shelves'].append({'x': x, 'y': display_y, 'type': selected_type})
...
def simulate_picker_movement():
    ...
    col1, col2, col3 = st.columns(3)
    with col1:
        num_pickers = st.slider("Number of Pickers", 1, 3, 1, key="picker_sim_num_pickers")
    with col2:
        num_orders = st.slider("Number of Orders", 1, 10, 3, key="picker_sim_num_orders")
    with col3:
        animation_speed = st.slider("Animation Speed (ms)", 100, 1000, 500, key="picker_sim_anim_speed")
...
def initialize_simulation():
    ...
    # Check if custom layout state exists, if not create a default layout
    if 'custom_layout_state' not in st.session_state:
        st.error("❌ No custom layout found. Please create a layout first or use the default layout.")
        return False
    ...
    # Get layout elements
    shelves = custom_state.get('shelves', [])
    stations = custom_state.get('stations', [])
    entry_exit = custom_state.get('entry_exit', None)
    ...
    num_pickers = st.session_state.get('picker_sim_num_pickers', 1)
    pickers = []
    ...
    orders = generate_random_orders(shelves, stations, entry_exit, st.session_state.get('picker_sim_num_orders', 3))
    
    st.session_state.picker_simulation.update({
        'pickers': pickers,
        'orders': orders,
        'current_step': 0,
        'animation_speed': st.session_state.get('picker_sim_anim_speed', 500)
    })
...
def display_animated_grid():
    ...
    # Check if custom layout state exists
    if 'custom_layout_state' not in st.session_state:
        st.error("❌ No custom layout found. Please create a layout first.")
        return
    
    grid_data = st.session_state.custom_layout_state.get('grid_data', [])
...
# Utility for random shelf type assignment (for testing)
def assign_random_shelf_types():
    import random
    type_codes = list(shelf_categories.keys())
    for shelf in st.session_state.custom_layout_state['shelves']:
        shelf['type'] = random.choice(type_codes) 